...
<!-- PROJECT LOGO -->
<h1>Constant Sum AMM Solidity Contract</h1>
...
We built an `AMM` smart contract, that depends on constant sum algorism (X + Y = K).
...
1. Clone the repo
   ```sh
   git clone https://github.com/Al-Qa-qa/constant-sum-amm.git
   ```
2. Install NPM packages (remember to move to the directory `constant-sum-amm` first)
   ```sh
   npm install
   ```
...
Project Link: [https://github.com/Al-Qa-qa/constant-sum-amm](https://github.com/Al-Qa-qa/constant-sum-amm)
