 ...
 */
async function deployToken0(chainId: number) {
  ...
  const token0Factory: Token0__factory = await ethers.getContractFactory("Token0", deployer);
  log("Deploying Contract...");
  const token0: Token0 = await token0Factory.deploy();
  await token0.deployed();

  log(`Token0 deployed to: ${token0.address}`);
  log("", "separator");

  if (!developmentChains.includes(network.name)) {
    // Verify Contract if it isnt in a development chain
    log("Verifying Contract", "title");
    await token0.deployTransaction.wait(VERIFICATION_BLOCK_CONFIRMATIONS);
    await verify(token0.address, []);
  ...
  oldContracts[network.name].Token0 = token0.address;
